...
export class EditAdminNavService {
  ...
  protected async updateNames({ id, name }: Pick<EditAdminNavArgs, 'name' | 'id'>) {
    ...
    Promise.all(
      names.map(async item => {
        ...
        await this.databaseService.db.delete(core_nav_name).where(eq(core_nav_name.id, item.id));
      })
    );
  ...
  protected async updateDescriptions({
    ...
    const descriptions = await this.databaseService.db.query.core_nav_description.findMany({
      where: (table, { eq }) => eq(table.nav_id, id)
    });

    const update = await Promise.all(
      description.map(async item => {
        const itemExist = descriptions.find(el => el.language_code === item.language_code);
  ...
  async edit({ description, external, href, id, name }: EditAdminNavArgs): Promise<ShowCoreNav> {
    ...
    const updatedDescription = await this.updateDescriptions({ id, description });
