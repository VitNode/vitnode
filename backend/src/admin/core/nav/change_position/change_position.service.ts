...
export class ChangePositionAdminNavService {
  ...
  async changePosition({ id, index_to_move, parent_id }: ChangePositionAdminNavArgs) {
    ...
    const allChildrenParent = await this.databaseService.db.query.core_nav.findMany({
      where: (table, { eq }) =>
