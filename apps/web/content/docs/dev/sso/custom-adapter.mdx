...
Implement the 5 interface methods and register the adapter in the `api` runtime of `vitnode.config.ts`:
...
<TypeTable
  type={{
    id: {
      description:
        "Unique slug for the provider (e.g. 'github', 'slack'). Used in callback URLs.",
      required: true,
      type: 'string',
    },
    name: {
    ...
    icon: {
      description:
        'Brand mark for the login button: inline SVG markup or an image URL. See Provider Icons.',
      required: false,
      type: 'string',
    },
    getUrl: {
    ...
    fetchUser: {
      description:
        'Fetches user profile (id, email, username, avatarUrl). Return only an email the provider has verified - VitNode treats it as confirmed and will mark a matching unconfirmed account verified. The built-in adapters reject unverified addresses with a 400.',
      required: true,
