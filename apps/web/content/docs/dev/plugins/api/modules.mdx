...
There is no registration step. `config.api` is one of the conventional subpaths
the API looks for under the plugin's package, so the module you just wrote is
picked up on the next start.

The app registered `siteNotesPlugin()` in `vitnode.config.ts` when it enabled
the pages, and that one registration covers the API too - there is nothing to
add on the app side.
