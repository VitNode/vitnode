...
`config.api` entry at startup. Nothing lists them: each is found at a
conventional subpath of the plugin's package, so adding `src/admin/nav.tsx`
later is enough to gain a sidebar entry. See
