...
People get two pictures: a round **avatar** shown next to their name everywhere,
and a wide **cover** across the top of their profile. Both are picked, cropped
and confirmed in the browser, then stored through the
[storage adapter](/docs/dev/storage) as ordinary `core_files` rows under the
`avatars` and `covers` folders.

They are deliberately **not** listed in the owner's *My Files*: the profile is
the one place to change or remove them, so removing one also clears it from the
account. Staff still see them in **Core → Files**, and deleting one there
clears the user's avatar or cover cleanly - the column is nulled, the cached
session is dropped, and the matching event fires. Should a URL ever go dead
anyway, the `Avatar` component falls back to the letter avatar and the cover
steps aside for the gradient.

{/* Image prompt: VitNode profile page with a cover photo across the top of the card, a round avatar overlapping it, and a small solid camera button on each. A dialog is open showing two radio options - upload and remove - above an image being cropped to a circle with a zoom slider underneath. Light theme, 1440x900. */}
