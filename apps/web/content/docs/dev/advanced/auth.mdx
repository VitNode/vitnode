...
Customize session parameters in the `api` runtime of `src/vitnode.config.ts`:
...
<Callout type="info" title="Session Isolation">
  Signing out of the AdminCP does not terminate the user's public session, and
  vice versa. An administrator compromised in a public context cannot access the
  AdminCP without re-authenticating with staff credentials.
</Callout>
...
<Callout type="info" title="Where it applies">
  Sign-up, sign-in, password reset, the SSO callback and AdminCP user management
  all resolve an address the same way. Someone who registered with a password at
  `jankowalski@gmail.com` and later clicks *Sign in with Google* - which hands
  back `jan.kowalski@gmail.com` - lands on their own account instead of a second
  one.
</Callout>
