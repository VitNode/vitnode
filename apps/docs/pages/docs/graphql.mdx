...
VitNode use [GraphQL](https://graphql.org/) to query and mutate data with [Codegen](https://the-guild.dev/graphql/codegen) to generate typescript types. It's a powerful tool that allows you to query only the data you need and get the data in the shape you want.

Before we start, we're recomending you to read the [GraphQL documentation](https://graphql.org/learn/) to understand the basics of the language.
