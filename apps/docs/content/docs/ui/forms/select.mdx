---
title: Select
description: Presents a dropdown list of options for the user to choose from, activated by clicking a button.
---

import { ImageZoom } from 'fumadocs-ui/components/image-zoom';
import img from '@/assets/ui/components/select.png';

<ImageZoom src={img} className="max-w-xs" alt="Select" />

<Callout title="Shadcn UI">
  This component is part of [Shadcn
  UI](https://ui.shadcn.com/docs/components/select).
</Callout>

## Usage

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

<Tabs items={['Auto Form', 'Manual']}>
  <Tab value="Auto Form">
```ts
import * as z from 'zod';
import { AutoForm } from 'vitnode-frontend/form/auto-form';
import { AutoFormSelect, AutoFormSelectProps } from 'vitnode-frontend/form/fields/select';
```

```ts
const formSchema = z.object({
  type: z.enum(['option-one', 'option-two']),
});

// or

const formSchema = z.object({
  type: z.nativeEnum(TestEnumProvider),
});
```

```tsx
<AutoForm
  formSchema={formSchema}
  filds={[
    {
      id: 'type',
      component: AutoFormSelect,
      componentProps: {
        placeholder: 'Select example',
        labels: {
          ['option-one']: 'Option One',
          ['option-two']: 'Option Two',
        },
      } as AutoFormSelectProps,
    },
  ]}
/>
```

  </Tab>

  <Tab value="Manual">
```ts
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from 'vitnode-frontend/components/ui/select';
```

```tsx
<Select>
  <SelectTrigger>
    <SelectValue placeholder="Select example" />
  </SelectTrigger>
  <SelectContent>
    <SelectItem value="option-one">Option One</SelectItem>
    <SelectItem value="option-two">Option Two</SelectItem>
  </SelectContent>
</Select>
```

  </Tab>
</Tabs>

## API Reference

[Radix UI Select](https://www.radix-ui.com/primitives/docs/components/select#api-reference)
