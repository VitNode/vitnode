...
You can pass arguments to the resolver function. For example, let's say we want to get a single category by its ID:
...
export class ShowBlogCategoriesResolver {
  ...
  async blog_categories__show(
    @Args('id', { type: () => Int }) id: number, // [!code highlight]
...
or you can use [Data Transfer Object (DTO)](/docs/dev/dto) for the arguments:
