...
export class CreateAdminTermsSettingsArgs {
  @Transform(TransformStringLanguageInput) // [!code highlight]
...
`TransformStringLanguageInput` is a custom decorator that will transform the input to the correct format.

To required translation fields you can use `@ArrayMinSize(2)` decorator.
