...
Create a schema `core_groups_names` in `schema.prisma` file for translation table.
The translation table must have:

- `language_id` with type `String` and `@db.VarChar(32)` value,
- `value` with type `String` and any string value,
- any fields for relation with main table (in our case `group_id`).

```prisma
model core_groups_names {
  id          String @id @unique @default(uuid())
  language_id String @db.VarChar(32)
  group_id    String
...
model core_groups {
  id        String                  @id @unique @default(uuid())
  name      core_groups_names[] @relation("core_groups_names_group")
  ...
}
```
...
```prisma
language core_languages @relation("core_groups_names_language", references: [id], fields: [language_id])
```
...
model core_groups_names {
  ...
  group    core_groups    @relation("core_groups_names_group", references: [id], fields: [group_id], onDelete: Cascade)
  language core_languages @relation("core_groups_names_language", references: [id], fields: [language_id])
...
<Callout type="info" emoji="ℹ️">
  For better performance you can add index to `language_id` field by adding `@@index([language_id])`
