...
Variables for `Database` and `Redis` are already set in `docker-compose-dev.yml` file with inside `backend` config as default values.
...
<Callout type="info" emoji="ℹ️">
  ...
  `.env` file and fill `FRONTEND_URL` variable _(For example
  `FRONTEND_URL=http://localhost`)_.
</Callout>
